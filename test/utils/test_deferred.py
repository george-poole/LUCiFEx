...
from lucifex.utils.py_utils.deferred import create_lazy_evaluator, DeferredBoolean

@pytest.mark.parametrize("arg", [1, 'one', (1.23, 4.56)])
def test_defer(arg):
    func = lambda x: x
    func_delayed = create_lazy_evaluator(func)
    assert func_delayed(arg)() == arg


def test_deferred_condition_with_mutable():
    x = [0]
    deferred = DeferredBoolean(lambda: x[0] > 0)
...
def test_deferred_condition_with_arg():
    deferred = DeferredBoolean(lambda x: x > 0)
