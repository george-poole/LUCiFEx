...
import numpy as np
from lucifex.fdm import advective_timestep
...
def test_advective_timestep_h_float(h):
    mesh = rectangle_mesh(2.0, 1.0, 10, 10)
    u = Function((mesh, 'DP', 0, 2))
    u.interpolate(lambda x: (x[0], x[1]))
    dt = advective_timestep(u, h)
...
@pytest.mark.parametrize("h", ['hmin', 'hmax'])
def test_advective_timestep_u_constant(ux, uy, h):
    mesh = rectangle_mesh(2.0, 1.0, 10, 10)
