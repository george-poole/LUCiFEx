...
&F_1(\widetilde{\textbf{u}}^{n+1}, \textbf{v}) = \int_\Omega\text{d}\Omega~\rho\textbf{v}\cdot\frac{\widetilde{\textbf{u}}^{n+1}-\textbf{u}^n}{\Delta t^n} + \rho \textbf{v}\cdot\mathcal{D}_{\textbf{u}}(\textbf{u}\cdot\nabla\textbf{u}) + \varepsilon(\textbf{v}):(-p^n\mathsf{I} + \mathcal{D}_\tau(\boldsymbol{\tau})) -\textbf{v}\cdot\mathcal{D}_{\textbf{f}}(\textbf{f}) \\ 
&-\int_{\partial\Omega}\textbf{v}\cdot(-p^n\mathsf{I} + \mathcal{D}_\tau(\boldsymbol{\tau}))\cdot\textbf{n} \\
...
$$
\begin{align*}
&\text{Find $\psi(\textbf{x}, t): \Omega\times[0, \infty) \to \mathbb{R}$ and $\omega(\textbf{x}, t): \Omega\times[0, \infty) \to \mathbb{R}$ such that } \\
&\begin{cases}
\nabla^2\psi =\omega & \\
