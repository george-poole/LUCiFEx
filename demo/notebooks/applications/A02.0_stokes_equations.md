# Stokes equations

## Mixed formulation

strong form of the boundary value problem
$$
\begin{align*}
&\text{Find $\textbf{u}(\textbf{x}): \Omega \to \mathbb{R}^d$ and $p(\textbf{x}): \Omega \to \mathbb{R}$ such that } \\
&\begin{cases}
\nabla\cdot\textbf{u} = 0 & \\
\textbf{0}=-\nabla p + \nabla\cdot\tau(\textbf{u}) + \textbf{f} & \forall\textbf{x}\in\Omega \\
\textbf{u} = \textbf{u}_{\text{E}} & \forall \textbf{x}\in\partial\Omega_{\text{E}} \\
(-p\mathsf{I}+\tau)\cdot\textbf{n} = \boldsymbol{\tau}_{\text{N}} & \forall\textbf{x}\in\partial\Omega_{\text{N}}=\partial\Omega/\partial\Omega_{\text{E}}
\end{cases}~.
\end{align*}
$$

weak form 
$$
\begin{align*}
&\text{Find $(\textbf{u}, p)\in V_{\textbf{u}} \times V_p$ such that } \\
&F(\textbf{u}, p;\textbf{v}, q)=\int_\Omega\text{d}\Omega~q(\nabla\cdot\textbf{u}) - p(\nabla\cdot\textbf{v}) + \nabla\textbf{v}:\tau - \textbf{v}\cdot\textbf{f} \\
&- \int_{\partial\Omega_{\text{N}}}\text{d}\Gamma~\,\textbf{v}\cdot\boldsymbol{\tau}_{\text{N}} \\
&=0 \quad\forall(\textbf{v}, q)\in V_{\textbf{u}} \times V_p~.
\end{align*}
$$

## Streamfunction formulation

general definition
$$\textbf{u}=\nabla\times\boldsymbol{\psi} \iff \nabla\cdot\textbf{u}=0$$

Newtonian stress with unit viscosity

$$\tau(\textbf{u}) = \tfrac{1}{2}\left(\nabla\textbf{u} + \nabla\textbf{u}^{\mathsf{T}}\right)$$

$$\implies\textbf{0}=\nabla^2(\nabla\times\boldsymbol{\psi}) + \nabla\times\textbf{f}$$

### Cartesian streamfunction in 2D

$$\textbf{u}=\nabla\times\psi\textbf{e}_z=-\frac{\partial\psi}{\partial y}\textbf{e}_x + \frac{\partial\psi}{\partial x}\textbf{e}_y$$

$$\textbf{f}=f_x\textbf{e}_x + f_y\textbf{e}_y$$

strong form of the boundary value problem
$$
\begin{align*}
&\text{Find $\psi(\textbf{x}): \Omega \to \mathbb{R}$ such that } \\
&\begin{cases}
\nabla^2(\nabla^2\psi) = \frac{\partial f_y}{\partial x}- \frac{\partial f_x}{\partial y} & \forall\textbf{x}\in\Omega \\
\psi=\psi_{\text{D}} & \forall \textbf{x}\in\partial\Omega \\
\nabla^2\psi=\psi_{\text{L}} & \forall \textbf{x}\in\partial\Omega
\end{cases}~.
\end{align*}
$$

weak form with penalty parameter $\alpha$ and local mesh cell size $\ell(\textbf{x})$
$$
\begin{align*}
&\text{Find $\psi\in V$ such that } \\
&F(\psi, v) = \int_\Omega\text{d}\Omega~\nabla^2v \nabla^2u - v\frac{\partial f_y}{\partial x} + v\frac{\partial f_x}{\partial y} \\ 
&+ \int_{\mathcal{F}}\text{d}\Gamma~\frac{\alpha}{\ell(\textbf{x})}\llbracket\nabla v\rrbracket\llbracket\nabla u\rrbracket - \llbracket\nabla v\rrbracket\langle\nabla^2u\rangle - \langle\nabla^2v\rangle\llbracket\nabla u\rrbracket \\ 
&=0\quad\forall v\in V~.
\end{align*}
$$