...
# def plot_colormaps(
#     f_series: Iterable[Function | tuple[np.ndarray, np.ndarray, np.ndarray]],
#     t_series: Iterable[float] | None = None,
#     field_name: str | None = None,
#     padding: tuple[float, float] | None = None,
#     n_cols: int = 3,
#     xy_labels: tuple[str, str] = ("$x$", "$y$"),
# ) -> tuple[Figure, Axes]:
...
#     if (t_series is not None) and (field_name is not None):
#         titles = [f"{field_name}(t={exponential_notation(t)})" for t in t_series]
#     else:
#         titles = [None] * len(f_series)

#     n_ax = len(f_series)

#     n_cols_choices = (3, 2)
#     rows_cols_set = False
#     for n in n_cols_choices:
#         if n_ax % n == 0:
#             n_cols = n
#             n_rows = int(n_ax / n)
#             rows_cols_set = True
...
#     if not n_ax % n_cols:
#         raise ValueError(
#             f"""Number of functions to plot must be a multiple of the 
#             number of columns {n_cols}"""
#         )
#     n_rows = int(n_ax / n)

#     fig, axs = subplots(n_rows, n_cols)

#     ax_indices = itertools.product(range(n_rows), range(n_cols))
#     xlabel, ylabel = xy_labels

#     for (irow, icol), f, title in zip(ax_indices, f_series, titles):
#         ax: Axes = axs[irow, icol]
#         _plot_colormap(f, fig_ax=(fig, ax))
#         if irow == n_rows - 1:
...
#         x_space, y_space = padding
#         fig.subplots_adjust(wspace=x_space, hspace=y_space)
